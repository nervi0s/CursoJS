/*1 - Descarga los pokemons del 1 al 150 en formato JSON desde la url http://80.211.144.192:9582/api/pokemon/1 .
**Define la clase Pokemon con todos los pokemons descargados, cuanto mejor definida y mas compleja sea mejor.
**Crear un sistema en el que 2 objetos de la misma clase puedan enfrentarse en una batalla.
**Es importante que la propiedad vida de los objetos se vea afectada segun si hay un ataque.
**PD: Cuanta mas creatividad despues de cumplir el objeto básico, mejor.*/

const axios = require('axios');
const fs = require('fs');


//obtenemos el objeto proporcionado por el método .get() de axios, concretamente obtenemos el valor del atributo data
...
//creamos 150 archivos .json con los datos de los primeros 250 pokemons
/* (async function () {
    for (let i = 1; i <= maxPokemonData; i++) {
        let urlApiPokemon = await getPekemonData("http://80.211.144.192:9582/api/pokemon/" + i);
        fs.writeFileSync("./PokemonScraped/" + i + "_" + "pokemon.json", JSON.stringify(urlApiPokemon));
        console.log("Escrito archivo",i,"de",maxPokemonData);
    }
})(); */
...
console.log("hay", arrayDeObjPokemon.length, "pokemons para elegir",);
